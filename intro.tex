...
% ~\cite{XiangSc15,XiangSc13} partitioned transactions ParT - planning phase and completion phase

% ~\cite{LesaniLuMo13} specifying semantics of TM w/ nontransactional code - tempting to use HTM whenever possible, 
% as it inherently interacts correctly
% wth nontransactional code - as though every 
% nontransactional instruction were an individual transaction.  
% Slow path is incurred when the HTM fails.
% Understanding the circumstances when this happens is essential 
% to reasoning about such design tradeoffs.

% ~\cite{LeisKeNe14} in-memory databases, uses Timestamp 
% Ordering~\cite{Carey83} to glue together smaller transactions.
% Assumes that read and write set must fit in L1, a misconception
% - in reality, a strong assymmetry in the size of the read and
% write sets exists in the Intel system.

% ~\cite{WangQiLi14} decompose in-memory db transactions into execution and commit, using RTM only for commit - transactional state scales as size of meta-data, not payload data
...
% ~\cite{WangGaWu12} unexpected cache locality effects from IBM's first
% implementation operating out of L2 - high startup costs and reduced 
% temporal locality in nontransactional sections.

% ~\cite{Drepper07} in the spirit of Drepper's characterization of 
% the general memory system.

% ~\cite{WangBuLi14} exploring design space of hybrid TM with 
% compiler technique like computation splitting and privatization
% - they do a small evaluation of Intel's TM limitations
